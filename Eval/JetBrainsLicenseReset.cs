...
internal class JetBrainsLicenseReset
{
    private readonly List<Tuple<string, string>> _folderPosition = new List<Tuple<string, string>>
    {
        ...
        new Tuple<string, string>(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                                  "JetBrains"),
    };
    ...
    private readonly List<Tuple<RegistryKeyType, string>> _registryPosition = new List<Tuple<RegistryKeyType, string>>
    {
        ...
        new Tuple<RegistryKeyType, string>(RegistryKeyType.Users,
                                           @"S-1-5-21-3093079274-4027988934-4014703911-1001\SOFTWARE\JavaSoft\Prefs\jetbrains"),
    };

    private readonly List<Tuple<RegistryKeyType, string>> _registrySpecialJavaPosition =
        new List<Tuple<RegistryKeyType, string>>
        {
    ...
    private bool ClearRegistryMemory(List<Tuple<RegistryKeyType, string>> positions)
    {
        ...
        foreach (var position in positions)
            ...
            {
                Log(
                    $"[Registry][Error] Delete key:{position.Item1} in path:{position.Item2} with exception: {e.Message}");
                allPathsDelete = false;
            }
    ...
    private RegistryKey GetRegistryKeyByType(RegistryKeyType registryKeyType)
    {
        switch (registryKeyType)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(nameof(registryKeyType), registryKeyType,
                                                      "Unsupported registry key Used");
        }
    }
